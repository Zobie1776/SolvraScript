...
NovaOS is a Rust workspace (`Cargo.toml`) with accompanying TypeScript packages (`pnpm-workspace.yaml`). Core runtimes live in `nova_core/` (VM, runtime, integration tests in `tests/`), language tooling in `nova_script/` (compiler, REPL), and automation features in `nova_ai/`. CLI and thin client layers are provided by `nova_cli/` and `nova_lite/`, while UI crates collect under `nova_ide/crates/*`. Generated artifacts land in `target/`; keep it out of version control.
...
- `cargo run -p nova_cli -- --help` — smoke-check the CLI entrypoint.
...
Rust code targets the toolchain pinned in `rust-toolchain.toml`; run `cargo fmt --all` and `cargo clippy --all-targets --all-features` before committing. Use 4-space indentation, snake_case files, and module trees that mirror directories (e.g., `nova_core/src/backends/mod.rs`). Frontend code follows `eslint.config.mjs`; keep Svelte components PascalCase under `nova_ide/` and shared utilities camelCase.
