## SolvraScript TextMate Grammar Quickstart

### Minimal Grammar Snippet (JSON)

```jsonc
{
  "scopeName": "source.solvrascript",
  "patterns": [
    { "include": "#comments" },
    { "include": "#strings" },
    { "include": "#numbers" },
  ...
  "repository": {
    "comments": {
      "patterns": [
        { "name": "comment.line.double-slash.solvrascript", "match": "//.*$" }
      ]
    },
    "strings": {
      "patterns": [
        {
          "name": "string.quoted.double.solvrascript",
          "begin": "\"",
          "end": "\"",
          "patterns": [
            { "name": "constant.character.escape.solvrascript", "match": "\\\\[ntr0\"\\]" }
          ]
        },
        {
          "name": "string.quoted.template.solvrascript",
          "begin": "`",
          "end": "`",
          "patterns": [
            {
              "name": "source.solvrascript.interpolated",
              "begin": "\\$\\{",
              "end": "}",
              "patterns": [{ "include": "source.solvrascript" }]
            }
          ]
        }
      ]
    ...
    "numbers": {
      "patterns": [
        { "name": "constant.numeric.solvrascript", "match": "\\b(?:0x[0-9a-fA-F]+|[0-9]+(?:\\.[0-9]+)?)\\b" }
      ]
    },
    "keywords": {
      "patterns": [
        {
          "name": "keyword.control.solvrascript",
          "match": "\\b(?:let|mut|const|fn|if|else|while|for|in|match|return|break|continue|try|catch|async|await)\\b"
        },
        {
          "name": "support.function.builtin.solvrascript",
...
1. Create a folder such as `.solvra-vscode/syntax` in your workspace.
2. Save the JSON snippet above as `solvrascript.tmLanguage.json` inside that folder.
3. Add a `package.json` to define a lightweight extension:

```json
{
  "name": "solvrascript-syntax",
  "displayName": "SolvraScript Syntax",
  "version": "0.0.1",
    ...
    "grammars": [
      {
        ...
        "scopeName": "source.solvrascript",
        "path": "./solvrascript.tmLanguage.json"
      }
    ],
    "languages": [
      {
        "id": "solvrascript",
        "aliases": ["SolvraScript"],
        "extensions": [".svs"],
...
6. Open any `.svs` file and run `Developer: Reload Window` to activate highlighting.
...
- Incrementally extend the `support.function.builtin.solvrascript` pattern as new built-ins are added.
