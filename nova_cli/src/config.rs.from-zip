//! Configuration handling for NovaCLI including loading and defaults.
...
impl Default for CliConfig {
    fn default() -> Self {
        Self {
            prompt: "\u{001b}[1;36mnova\u{001b}[0m> ".to_string(),
            aliases: HashMap::new(),
            env: HashMap::new(),
        }
    }
...
impl CliConfig {
    ...
    pub fn load() -> anyhow::Result<(Self, PathBuf)> {
        let dirs = ProjectDirs::from("dev", "Nova", "nova")
            .ok_or_else(|| anyhow::anyhow!("unable to determine configuration directory"))?;
        let config_dir = dirs.config_dir();
        fs::create_dir_all(config_dir).context("creating nova config directory")?;
