# NovaCore Advanced Roadmap

This document outlines the staged plan for evolving NovaCore into the feature-complete runtime described in the latest engineering brief. Each stage is scoped so that we can deliver, review, and test one file (or tightly scoped module) at a time while keeping the codebase buildable.
...
1. `src/backend/ir.rs`: Define a Nova SSA/IR representation, instruction set, and builder APIs.
2. `src/backend/lowering.rs`: Lower existing Nova bytecode into the IR, handling control-flow graphs and phi nodes.
...
2. `runtime/syscalls/mod.rs`: Provide NovaScript-visible system call API bridging to HAL.
3. Update NovaScript standard library modules to exercise new operations (tests under `nova_script/tests`).
...
1. `docs/IR_SPEC.md`: Formal specification of the Nova IR.
...
4. `examples/`: Add advanced NovaScript samples (system calls, concurrency, device interaction) covering the `.ns` test checklist.
