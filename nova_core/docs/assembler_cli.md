...
steps below show how to assemble `.nvc` programs using the crate.
...
fn assemble_sample() -> anyhow::Result<()> {
    ...
    std::fs::write("hello.nvc", bytecode.into_bytes())?;
    Ok(())
}
```

You can run the snippet with `cargo run --example embed_runtime` and tailor the
AST to match your program.  Projects that already produce `.nvc` text files
(such as the fixtures in `samples/`) can parse the directives and emit the
binary layout defined in [`nvc_format.md`](./nvc_format.md) before writing
out the compiled artifact.

## Inspecting Output

The generated bytecode is self-contained.  Use `hexdump -C hello.nvc` or a
...
`nova_core/build/` to cross-compile NovaCore and rebuild `.nvc` artifacts in a
