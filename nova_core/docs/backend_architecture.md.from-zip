...
NovaCore now exposes a modular backend interface that makes it possible to
...
│   ├── x86_64/      # Native x86_64 backend backed by the bytecode interpreter
...
```bash
cargo build -p nova_core
```

To target ARMv7:

```bash
cargo build -p nova_core --no-default-features --features backend-armv7
```

To target AArch64:

```bash
cargo build -p nova_core --no-default-features --features backend-aarch64
```
...
cargo build -p nova_core --no-default-features --features backend-armv7 --target armv7-unknown-linux-gnueabihf
```

The interpreter-based backends currently emit Nova bytecode and execute it using
...
The [`NovaRuntime`](../src/lib.rs) type exposes helper methods to introspect the
selected backend:

* `NovaRuntime::backend()` returns the backend implementation.
* `NovaRuntime::target_arch()` returns the active [`TargetArch`](../src/backend/mod.rs) value.
...
4. Update `backend::active_backend` to select the new backend when its feature is
   active.
5. Add tests under `nova_core/tests/` gated behind the new feature flag to ensure
   the backend can execute sample programs.
6. Update CI to exercise the backend with `cargo test` and, if applicable,
   `cargo clippy`.

Following this process keeps NovaCore extensible and ensures every backend ships
