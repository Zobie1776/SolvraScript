# NovaCore Backend Modularization Plan
...
* Introduce an explicit backend trait that defines the contract a backend must
  satisfy to integrate with NovaCore.
...
The refactor introduces a new `backend` namespace under `nova_core/src`:

```
nova_core/src/backend/
...
pub trait ArchitectureBackend {
    ...
    /// Lowers Nova bytecode or IR into a backend-specific artifact (native
...
* `BackendInput` will initially wrap `NovaBytecode`.  The abstraction keeps the
...
* `NovaRuntime` gains a `backend: Arc<dyn ArchitectureBackend>` field.
...
* Integration tests running sample NovaScript programs through each backend.
