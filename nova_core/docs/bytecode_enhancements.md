# NovaCore Bytecode Enhancements Roadmap

1. **Instruction Set Review** – catalogue existing opcodes, tag gaps for device IO, and add
   meta-information for NovaAI-aware diagnostics.
...
4. **Deterministic Sandbox Mode** – add cost counters per instruction class and expose to NovaLite.
5. **Testing Strategy** – craft `.nvc` fixtures (see `tests/nvc_samples`) and interpreter-driven
