# NovaCore `.novac` File Format

The NovaCore runtime consumes a compact binary representation of Nova bytecode.
`*.novac` files follow a strict layout so that the interpreter and ahead-of-time
...
`NovaError::RuntimeException` failures.
...
`nova_core/samples/` use directives such as `.version`, `.entry`, `.const` and
...
representation described above, typically by the higher level NovaScript
compiler or by bespoke tooling built with `nova_core::bytecode::assembler`.

## Reference Samples

* `samples/hello_world.novac` – prints a greeting via `println`
* `samples/add_numbers.novac` – defines an `add` helper and returns `2 + 3`
* `samples/loop_counter.novac` – iterates from `0` to `5`, logging each value
* `samples/virtual_device_driver.novac` – registers a mock driver and exposes a
