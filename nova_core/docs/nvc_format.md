# NovaCore `.nvc` File Format

The NovaCore runtime consumes a compact binary representation of Nova bytecode.
`*.nvc` files follow a strict layout so that the interpreter and ahead-of-time
...
* `samples/hello_world.nvc` – prints a greeting via `println`
* `samples/add_numbers.nvc` – defines an `add` helper and returns `2 + 3`
* `samples/loop_counter.nvc` – iterates from `0` to `5`, logging each value
* `samples/virtual_device_driver.nvc` – registers a mock driver and exposes a
