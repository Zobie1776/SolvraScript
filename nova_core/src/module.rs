...
impl Module {
    pub fn new(name: impl Into<String>, bytecode: NovaBytecode) -> Self {
        Self {
            name: name.into(),
    ...
    pub fn bytecode(&self) -> Arc<NovaBytecode> {
        self.bytecode.clone()
    }
}
...
impl ModuleLoader {
    ...
    pub fn load_bytes(&mut self, name: impl Into<String>, bytes: &[u8]) -> NovaResult<Arc<Module>> {
        let name = name.into();
    ...
    pub fn load_file(&mut self, path: impl AsRef<Path>) -> NovaResult<Arc<Module>> {
