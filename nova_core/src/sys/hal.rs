//! Hardware Abstraction Layer definitions for NovaCore.
//!
//! The HAL acts as a thin shim between architecture-specific backends,
//! device drivers, and the NovaRuntime.  It provides a trait-based
...
/// Storage bus variants supported by NovaCore.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum StorageBus {
    Sata,
    Nvme,
...
/// Sensor types supported by NovaCore.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum SensorKind {
    Temperature,
    Motion,
...
impl SoftwareHal {
    ...
    /// Registers the default virtual devices used by NovaRuntime in sandboxed mode.
