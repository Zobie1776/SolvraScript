...
fn builtins_file_io() {
    ...
    let path = temp.path().join("nova_runtime.txt");
        ...
        vec![
            Stmt::Expr(
                Expr::call(
                    ...
                    vec![Expr::string(&path_str), Expr::string("nova")],
                ),
                span.clone(),
            ),
            Stmt::Return(
    ...
    assert_eq!(value, Value::String("nova".into()));
}
...
fn reports_stack_trace_on_error() {
    ...
    match err {
        NovaError::RuntimeException { stack, .. } => {
