//=============================================
// nova_shell_launcher/src/main.rs
//=============================================
// Author: Nova GUI Team
// License: MIT
// Goal: Nova GUI desktop surface
...
/// Top-level iced application driving the Nova desktop UI.
struct DesktopApp {
    tokens: ThemeTokens,
    layout: LayoutInfo,
    style: StyleCatalog,
...
impl WindowEntry {
    ...
    fn view(&self, style: &StyleCatalog) -> Element<Message> {
        let contents = match self.kind {
            ...
            WindowKind::Ide => app_panel("Nova IDE", "Craft code with AI assistance.", style),
            WindowKind::Cli => app_panel("Nova CLI", "Automate your workflows quickly.", style),
...
impl WindowKind {
    fn title(&self) -> &'static str {
        match self {
            WindowKind::Overview => "Nova Overview",
            WindowKind::Ide => "Nova IDE",
            WindowKind::Cli => "Command Line",
            WindowKind::AppStore => "Nova App Store",
...
fn overview_panel(style: &StyleCatalog) -> Element<'static, Message> {
    let headline = text("Welcome to Nova OS")
        .size(32)
        .style(style.text_primary_style())
