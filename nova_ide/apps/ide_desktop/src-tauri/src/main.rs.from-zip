...
use anyhow::Context;
use nova_fail_safe::{register_passphrase, verify};
use nova_ide_core::lsp::{
    CompletionItem, Diagnostic, HoverResult, NovaLanguageServer, TextPosition,
};
use nova_ide_core::tasks::{RunTaskPayload, TaskOutcome, TaskRunner};
use nova_ide_core::tree::{ProjectNode, ProjectTreeBuilder};
...
type SharedLsp = Mutex<NovaLanguageServer>;
...
pub fn run() {
    ...
    tauri::Builder::default()
        .manage(Mutex::new(NovaLanguageServer::new()))
        ...
        .expect("error while running NovaIDE desktop");
}
