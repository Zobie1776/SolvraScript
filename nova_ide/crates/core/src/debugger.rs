...
impl DebugSession {
    ...
    pub fn step(&mut self, file: PathBuf, line: u32) -> Result<DebuggerEvent, NovaIdeError> {
    ...
    pub fn hit_breakpoint(
        ...
        file: PathBuf,
        line: u32,
    ) -> Result<DebuggerEvent, NovaIdeError> {
        if !self.has_breakpoint(&file, line) {
            return Err(NovaIdeError::debugger("breakpoint missing"));
        }
        self.state.current_file = Some(file.clone());
        self.state.current_line = Some(line);
...
mod tests {
    ...
    fn toggles_breakpoint() {
        ...
        let file = PathBuf::from("main.nova");
