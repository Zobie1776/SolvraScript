...
//! NovaFailSafe – lightweight publish protection for NovaIDE.
//!
...
impl FailSafeConfig {
    /// Serialises the config into TOML for easy storage inside NovaIDE.
...
mod tests {
    ...
    fn register_and_verify() {
        let config = register_passphrase("nova").expect("can register");
        assert!(verify("nova").is_ok());
        assert_eq!(
