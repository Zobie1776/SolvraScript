...
use crate::nova_ai::{CompletionContext, NovaAiService};
...
pub trait IdePlugin: Send {
    ...
    fn show(&mut self, ctx: &egui::Context, app: &mut NovaIdeApp);
}

pub struct NovaIdeApp {
    pub context: NovaIdeContext,
    pub file_explorer: FileExplorer,
    pub editor: EditorState,
    pub terminal: TerminalPane,
    pub diagnostics: Vec<DiagnosticItem>,
    pub git_panel: GitPanelState,
    pub ai_service: NovaAiService,
...
impl NovaIdeApp {
    fn new(context: NovaIdeContext) -> Self {
        ...
        let ai_service = NovaAiService::default();
    ...
    fn render_menu_bar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            ui.checkbox(&mut self.show_ai_panel, "NovaAI");
    ...
    fn render_ai_panel(&mut self, ui: &mut egui::Ui) {
        ui.heading("NovaAI");
        if !self.settings.data.enable_nova_ai {
            ui.label("NovaAI is disabled in settings.");
            return;
        }
        ...
        if self.show_settings {
            egui::Window::new("Settings")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.checkbox(&mut self.settings.data.enable_nova_ai, "Enable NovaAI");
                });
        }
    }
    ...
    fn command_entries(&self) -> Vec<CommandPaletteEntry> {
        let mut entries = vec![
            ...
            CommandPaletteEntry::new("Toggle NovaAI", |app| {
...
pub struct CommandPaletteEntry {
    pub label: String,
    pub action: Box<dyn Fn(&mut NovaIdeApp) + Send + Sync>,
}

impl CommandPaletteEntry {
    fn new(
        label: impl Into<String>,
        action: impl Fn(&mut NovaIdeApp) + Send + Sync + 'static,
    ) -> Self {
        Self {
            label: label.into(),
...
pub fn run_gui(options: GuiLaunchOptions) -> eframe::Result<()> {
    ...
    let mut context = Some(NovaIdeContext {
    ...
    eframe::run_native(
        "NovaIDE v2",
        native_options,
        Box::new(move |_cc| Box::new(NovaIdeApp::new(context.take().expect("app context")))),
    )
}
