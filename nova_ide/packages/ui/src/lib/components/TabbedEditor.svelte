<script lang="ts">
  ...
  function defineThemes() {
    if (!monaco) return;
    monaco.editor.defineTheme('nova-solarized', {
      base: 'vs-dark',
      inherit: true,
      rules: [],
      colors: {
  ...
  function applyTheme(value: typeof theme) {
    if (!monaco) return;
    const themeKey = value === 'solarized' ? 'nova-solarized' : value === 'dark' ? 'vs-dark' : 'vs';
  ...
  function updateEditorModel() {
    ...
    if (!model) {
      ...
      model = monaco.editor.createModel(tab.content, 'novascript', uri);
  ...
  function configureCompletions() {
    ...
    completionDisposable?.dispose();
    completionDisposable = monaco.languages.registerCompletionItemProvider('novascript', {
      triggerCharacters: ['.', ':'],
