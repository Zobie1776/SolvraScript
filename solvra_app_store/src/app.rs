...
pub struct AppPackage {
    ...
    /// Capabilities exposed by the application to the rest of SolvraOS.
    pub capabilities: Vec<AppCapability>,
    /// UI components that can be embedded into SolvraShell / SolvraIDE.
...
/// Describes a UI component that can be embedded into SolvraOS surfaces.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
impl UiComponent {
    ...
    /// Provide a concrete entry point (such as a SolvraScript or Web component).
...
/// Enumerates supported UI integration points in SolvraOS surfaces.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum UiComponentKind {
    /// A dedicated workspace tab inside SolvraIDE.
    IdeView,
    /// A widget rendered inside SolvraShell dashboards.
    ShellWidget,
