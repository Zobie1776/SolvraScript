...
/// Return catalog metadata for SolvraStream.
pub fn metadata() -> AppMetadata {
    let sandbox = SandboxPolicy::new()
        ...
        .allow_network_host("rtmp://live.solvraos.dev")
        .allow_storage_root("~/Videos/SolvraStream");

    let package = AppPackage::new(Version::new(1, 0, 0))
        .with_sandbox(sandbox)
        .with_capability(
            AppCapability::new(
                "solvrastream.capture",
        ...
        .with_ui_component(
            ...
            .with_entry_point("solvrastream::studio"),
        );

    AppMetadata::new(
        AppId::new("dev.solvra.stream").expect("valid id"),
        "SolvraStream",
        "Stream, record, and mix multiple media sources",
        "SolvraStream delivers an OBS-inspired workflow with scene presets, capture device management, and extensible output pipelines.",
    ...
    .with_tag("video")
    .with_tag("streaming")
    .with_screenshot("screenshots/solvrastream.png")
}
...
/// Capture sources supported by SolvraStream.
