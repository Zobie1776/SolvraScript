...
use solvra_app_store::{AppId, AppStore};
use std::str::FromStr;

/// Built-in command providing access to the Solvra App Store from the CLI.
...
impl CommandHandler for AppStoreCommand {
    ...
    fn summary(&self) -> &str {
        "Interact with the Solvra App Store (catalog, install, uninstall)"
    }
