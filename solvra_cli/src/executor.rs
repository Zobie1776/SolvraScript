...
use solvrascript::interpreter::Interpreter;
use solvrascript::parser::Parser as SolvraParser;
use solvrascript::tokenizer::Tokenizer as SolvraTokenizer;
use std::io::Write;
...
/// Executor driving command dispatch and SolvraScript evaluation.
pub struct Executor {
...
impl Executor {
    ...
    pub fn execute(&mut self, statement: Statement) -> Result<CommandOutcome> {
        match statement {
            ...
            Statement::SolvraBlock(src) => self.execute_solvra_block(&src),
            Statement::SolvraExpression(expr) => self.execute_solvra_expr(&expr),
    ...
    fn execute_solvra_block(&mut self, src: &str) -> Result<CommandOutcome> {
        let mut tokenizer = SolvraTokenizer::new(src);
        let tokens = tokenizer.tokenize().map_err(|err| anyhow::anyhow!(err))?;
        let mut parser = SolvraParser::new(tokens);
        let program = parser.parse().map_err(|err| anyhow::anyhow!(err))?;
        let result = self
            .interpreter
            .eval_program(&program)
            .map_err(|e| anyhow::anyhow!("SolvraScript error: {:?}", e))?;
    ...
    fn execute_solvra_expr(&mut self, expr: &str) -> Result<CommandOutcome> {
        let mut tokenizer = SolvraTokenizer::new(expr);
        let tokens = tokenizer.tokenize().map_err(|err| anyhow::anyhow!(err))?;
        let mut parser = SolvraParser::new(tokens);
        let program = parser.parse().map_err(|err| anyhow::anyhow!(err))?;
        let result = self
    ...
    fn execute_pipeline(&mut self, mut pipeline: Pipeline) -> Result<CommandOutcome> {
        ...
        for (index, command) in pipeline.commands_mut().iter_mut().enumerate() {
            ...
            let outcome = match resolution {
                ...
                Some((CommandKind::Solvra, handler)) => handler.handle(
                    &mut ctx,
                    &CommandInvocation::new(program.clone(), args.clone(), stdin_data.clone()),
                )?,
