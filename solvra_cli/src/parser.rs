//! Parser that understands SolvraCLI command syntax including pipes and expansions.
...
impl Parser {
    ...
    pub fn parse(&mut self, input: &str) -> Result<Statement> {
        ...
        if let Some(expr) = trimmed.strip_prefix("=>") {
            return Ok(Statement::SolvraExpression(expr.trim().to_string()));
        }
        if trimmed.starts_with("sv") {
            if let Some(block) = Self::extract_solvra_block(input)? {
                return Ok(Statement::SolvraBlock(block));
            }
        }
        self.parse_pipeline(input)
    }

    fn extract_solvra_block(input: &str) -> Result<Option<String>> {
        let mut chars = input.chars().peekable();
        skip_whitespace(&mut chars);
        if !consume_literal(&mut chars, "sv") {
            return Ok(None);
        }
        skip_whitespace(&mut chars);
        if chars.next() != Some('{') {
        ...
        bail!("unterminated SolvraScript block")
    }
...
mod tests {
    ...
    #[test]
    fn parses_solvra_block() {
        let mut parser = Parser::new();
        let stmt = parser
            .parse("sv {\nlet x: int = 1;\n}")
            .expect("Solvra block");
        assert!(matches!(stmt, Statement::SolvraBlock(_)));
    }

    #[test]
    fn parses_solvra_expression() {
        let mut parser = Parser::new();
        let stmt = parser.parse("=> 1 + 1").unwrap();
        assert!(matches!(stmt, Statement::SolvraExpression(_)));
    }
}
