...
use crate::{SolvraResult, RuntimeConfig, Value};
...
impl BackendArtifact {
    ...
    pub fn bytecode(&self) -> Option<Arc<SolvraBytecode>> {
        match self {
            BackendArtifact::Bytecode(code) => Some(code.clone()),
            BackendArtifact::SativeBlob(_) => None,
        }
    }
}
...
pub trait ArchitectureBackend: Send + Sync {
    ...
    /// Lowers Solvra bytecode into a backend specific artifact.
    fn compile(&self, bytecode: Arc<SolvraBytecode>) -> SolvraResult<BackendArtifact>;
    ...
    fn optimise(&self, _bytecode: &mut SolvraBytecode) -> Result<()> {
        Ok(())
    }
}
