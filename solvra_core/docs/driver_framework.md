# SolvraCore Driver Framework Overview

The driver framework unifies host-native peripherals and SolvraCore modules.  It
provides shared register storage, interrupt queues, and telemetry hooks that
allow SolvraShell, SolvraIDE, and other host tools to introspect device behaviour.
...
* `DriverDescriptor` lets host applications create devices with pre-filled
  registers prior to executing SolvraCore bytecode.
...
```rust
use solvra_core::{DriverDescriptor, SolvraRuntime};
...
Every public API returns `SolvraResult<T>` so errors integrate with existing
runtime diagnostics (`SolvraError::Sative`).

## SolvraCore Sative Bindings

Driver bindings are exposed as native functions callable from SolvraCore modules
and `.svc` programs:
...
These bindings are available immediately after `SolvraRuntime::new()` because the
VM initialises its native registry with driver functions.  The sample program in
`samples/virtual_device_driver.svc` demonstrates how to register a device,
...
SolvraShell and SolvraIDE can attach listeners via `runtime.hooks().set_telemetry(...)`
to display device dashboards, while `set_logger(...)` surfaces structured log
messages such as `driver[virtual_temp] = 1`.

## Testing Strategy

Unit tests in `solvra_core/tests/drivers.rs` assemble SolvraCore AST programs that
...
* Host-queued interrupts become visible to SolvraCore modules via
  `driver_next_interrupt`.
* Interrupts raised from bytecode are observable from host code by querying the
  registry.

These tests leverage the same APIs available to SolvraShell/SolvraIDE and guard the
