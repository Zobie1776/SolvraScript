...
use solvra_core::{SolvraRuntime, RuntimeRepl, Value};
...
fn main() {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--version" | "-V" => {
                println!("solvra-runtime {}", env!("CARGO_PKG_VERSION"));
                return;
            }
            "--repl" => {
                run_repl = true;
...
fn launch_repl(runtime: SolvraRuntime) {
...
fn print_help() {
    println!("Solvra runtime tool\n");
    println!("USAGE:");
    println!("    solvra-runtime [OPTIONS] [FILE]\n");
    println!("OPTIONS:");
    println!("    -h, --help         Print this message");
    println!("    -V, --version      Print version information");
    println!("        --exec <FILE>  Execute the provided .svc bytecode file");
    println!("        --load <FILE>  Load a .svc module before entering the REPL (repeatable)");
