...
use crate::backend;
use crate::nvc;
use crate::runtime::{SolvraError, SolvraRuntime};
use crate::SolvraResult;

/// Interactive REPL helper bound to a [`SolvraRuntime`].
pub struct RuntimeRepl {
    runtime: SolvraRuntime,
}

impl RuntimeRepl {
    /// Creates a new REPL wrapper around the provided runtime.
    pub fn new(runtime: SolvraRuntime) -> Self {
        Self { runtime }
    }

    /// Launches the REPL using standard input and output streams.
    pub fn run(&mut self) -> SolvraResult<()> {
        let stdin = io::stdin();
    ...
    pub fn run_with<R: BufRead, W: Write>(
        ...
        writeln!(writer, "Solvra runtime REPL. Type :help for commands.").map_err(map_io_error)?;
        let mut line = String::new();
        loop {
            writer.write_all(b"solvra> ").map_err(map_io_error)?;
    ...
    fn handle_command<R: BufRead, W: Write>(
        ...
        match command {
            ":help" => {
                ...
                writeln!(writer, "  :load <path>    Load a .svc module into memory")
                    .map_err(map_io_error)?;
                writeln!(
                    writer,
                    "  :exec <path>    Execute a .svc binary file immediately"
                )
                .map_err(map_io_error)?;
                writeln!(
                    writer,
                ...
                writeln!(
                    writer,
                    "  :asm            Enter multi-line Solvra assembly (finish with :end)"
                )
                .map_err(map_io_error)?;
                writeln!(
                    writer,
                    "  <assembly>      Single line Solvra assembly executed immediately"
                )
                .map_err(map_io_error)?;
                Ok(true)
            }
            ...
            ":asm" => {
                writeln!(
                    writer,
                    "Enter Solvra assembly. Finish input with a single line containing :end."
                )
                .map_err(map_io_error)?;
    ...
    fn execute_assembly<W: Write>(&self, source: &str, writer: &mut W) -> SolvraResult<()> {
        match solvrac::assemble(source) {
...
fn map_io_error(err: io::Error) -> SolvraError {
    SolvraError::Sative(err.to_string())
}
