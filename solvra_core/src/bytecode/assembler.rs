...
use super::{
    ...
    spec::{
        Constant, DebugSymbol, FunctionDescriptor, Instruction, Opcode, SolvraBytecode,
        SolvraBytecodeError,
    },
};
...
pub fn assemble(ast: &Ast) -> Result<SolvraBytecode, SolvraBytecodeError> {
...
) -> Result<SolvraBytecode, SolvraBytecodeError> {
...
impl<'a> Assembler<'a> {
    ...
    fn compile(&mut self) -> Result<SolvraBytecode, SolvraBytecodeError> {
    ...
    fn register_function(&mut self, name: &str, arity: usize) -> Result<(), SolvraBytecodeError> {
        if self.function_map.contains_key(name) {
            return Err(SolvraBytecodeError::Assembly(format!(
    ...
    fn compile_function(
        ...
        body: &[Stmt],
        span: &Span,
    ) -> Result<IrFunction, SolvraBytecodeError> {
        let mut state =
    ...
    fn lower(&self, program: IrProgram) -> Result<SolvraBytecode, SolvraBytecodeError> {
...
impl<'a> FunctionState<'a> {
    fn new(
        ...
        function_map: &'a HashMap<String, usize>,
        builtins: &'a Builtins,
    ) -> Result<Self, SolvraBytecodeError> {
    ...
    fn finish(self) -> Result<IrFunction, SolvraBytecodeError> {
        Ok(IrFunction::new(
            self.name,
            self.arity,
            self.locals,
    ...
    fn declare_parameter(&mut self, name: &str, index: u16) -> Result<(), SolvraBytecodeError> {
    ...
    fn declare_local(&mut self, name: &str, span: &Span) -> Result<u16, SolvraBytecodeError> {
    ...
    fn compile_block(&mut self, statements: &[Stmt]) -> Result<(), SolvraBytecodeError> {
    ...
    fn compile_stmt(&mut self, stmt: &Stmt) -> Result<(), SolvraBytecodeError> {
        match stmt {
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Result<(), SolvraBytecodeError> {
        match &expr.kind {
            ...
            ExprKind::Call { callee, args } => match &callee.kind {
                ExprKind::Identifier(name) => {
                    ...
                    let args_len = u16::try_from(args.len()).map_err(|_| {
                        SolvraBytecodeError::Assembly("too many call arguments".into())
                    })?;
                    if let Some(index) = self.function_map.get(name) {
                        ...
                        self.emit(
                            IrOpcode::Call {
                                target: CallTarget::Sative(index),
                                args: args_len,
                            },
                            &expr.span,
                        );
                ...
                _ => {
                    return Err(SolvraBytecodeError::Assembly(
            ...
            ExprKind::List(elements) => {
                ...
                let count = u16::try_from(elements.len()).map_err(|_| {
                    SolvraBytecodeError::Assembly("list literal exceeds limit".into())
                })?;
...
) -> Result<FunctionDescriptor, SolvraBytecodeError> {
    use IrOpcode::*;
        ...
        match &inst.opcode {
            ...
            Jump(label) => {
                let Some(target) = label_positions.get(&label.0) else {
                    return Err(SolvraBytecodeError::Assembly("dangling jump label".into()));
                };
            ...
            Call { target, args } => match target {
                CallTarget::Function(index) => instructions.push(Instruction::new(
                    ...
                    u32::try_from(*index).map_err(|_| {
                        SolvraBytecodeError::Assembly("function index overflow".into())
                    })?,
                    u32::from(*args),
                    debug_index,
                )),
                CallTarget::Sative(index) => instructions.push(Instruction::new(
                    Opcode::CallSative,
                    u32::try_from(*index).map_err(|_| {
                        SolvraBytecodeError::Assembly("native index overflow".into())
                    })?,
                    u32::from(*args),
                    debug_index,
                )),
            ...
            PushCatch { handler } => {
                let Some(target) = label_positions.get(&handler.0) else {
                    return Err(SolvraBytecodeError::Assembly("dangling catch label".into()));
                };
