//! Abstract syntax tree representation consumed by the SolvraCore assembler.
//!
//! The AST mirrors the high level constructs supported by SolvraScript.  It purposefully keeps
//! expression semantics simple so the assembler can perform deterministic lowering into the
//! SolvraCore intermediate representation.
...
/// Statements supported by the SolvraScript subset consumed by SolvraCore.
#[derive(Debug, Clone, PartialEq)]
pub enum Stmt {
    Let {
        name: String,
...
/// Expressions supported by the SolvraScript subset.
#[derive(Debug, Clone, PartialEq)]
pub struct Expr {
    pub kind: ExprKind,
    pub span: Span,
