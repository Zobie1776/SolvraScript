//! Definition of the SolvraBytecode binary format.
...
/// Opcodes understood by the SolvraCore virtual machine.
...
impl TryFrom<u8> for Opcode {
    type Error = SolvraBytecodeError;
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        ...
        let opcode = match value {
            ...
            33 => Throw,
            34 => DebugTrap,
            other => return Err(SolvraBytecodeError::UnknownOpcode(other)),
        };
        Ok(opcode)
    }
}

/// Single instruction stored in SolvraBytecode.
...
/// In memory representation of SolvraBytecode.
#[derive(Debug, Clone)]
pub struct SolvraBytecode {
...
impl SolvraBytecode {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, SolvraBytecodeError> {
        if bytes.len() < 10 {
            return Err(SolvraBytecodeError::UnexpectedEof);
        }
        if &bytes[0..4] != MAGIC {
            return Err(SolvraBytecodeError::InvalidMagic);
        }
        let version = u16::from_le_bytes([bytes[4], bytes[5]]);
        if version != VERSION {
            return Err(SolvraBytecodeError::UnsupportedVersion(version));
        }
        let mut cursor = 6;
            ...
            let constant = match tag {
                ...
                4 => {
                    ...
                    let text = String::from_utf8(slice.to_vec()).map_err(|err| {
                        SolvraBytecodeError::InvalidUtf8(err.utf8_error().valid_up_to())
                    })?;
                    Constant::String(text)
                }
                other => return Err(SolvraBytecodeError::UnknownConstantTag(other)),
            };
        ...
        for _ in 0..debug_len {
            ...
            let name = String::from_utf8(bytes[cursor..cursor + name_len].to_vec())
                .map_err(|err| SolvraBytecodeError::InvalidUtf8(err.utf8_error().valid_up_to()))?;
            cursor += name_len;
        ...
        for _ in 0..functions_len {
            ...
            for _ in 0..inst_len {
                ...
                let debug = if debug_idx == u32::MAX {
                    ...
                    return Err(SolvraBytecodeError::InvalidDebugSymbol(debug_idx));
                };
