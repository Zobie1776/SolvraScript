...
use crate::{RuntimeConfig, SolvraError, SolvraResult, StackFrame, Value};

use super::spec::{Constant, DebugSymbol, Opcode, SolvraBytecode};

/// Virtual machine executing SolvraBytecode instructions.
pub struct Vm {
...
impl Vm {
    ...
    fn run(&mut self) -> SolvraResult<Value> {
        loop {
    ...
    fn step(&mut self) -> SolvraResult<()> {
        self.consumed_cost += 1;
        if self.consumed_cost > self.cost_limit {
            Err(SolvraError::CostLimitExceeded)
        } else {
    ...
    fn pop(&mut self) -> SolvraResult<Value> {
        self.stack.pop().ok_or(SolvraError::StackUnderflow)
    }

    fn call_function(&mut self, function_index: usize, args: usize) -> SolvraResult<()> {
        let function = self
            .bytecode
            .functions()
            .get(function_index)
            .ok_or_else(|| SolvraError::Internal("call to unknown function".into()))?;
        ...
        if args > self.stack.len() {
            return Err(SolvraError::StackUnderflow);
        }
            ...
            Value::Object(reference) => {
                ...
                let error = if let Some(object) = self.gc.get_mut(reference) {
                    match object {
                        ...
                        GcObject::Sative(_) => Some("cannot mutate native object via index"),
                    }
                } else {
    ...
    fn raise(&mut self, value: Value, debug: Option<u32>) -> SolvraResult<()> {
    ...
    fn read_string_constant(&self, index: usize) -> SolvraResult<&str> {
        match self.bytecode.constants().get(index) {
            Some(Constant::String(text)) => Ok(text),
            _ => Err(SolvraError::Internal("global name must be a string".into())),
        }
    }
...
type SativeHandler = Arc<dyn Fn(&mut Vm, &[Value]) -> Result<Value, String> + Send + Sync>;

#[derive(Clone)]
struct SativeFunction {
    name: &'static str,
    handler: SativeHandler,
    arity: usize,
}

#[derive(Clone)]
struct SativeRegistry {
    functions: Vec<SativeFunction>,
}
...
impl SativeRegistry {
    fn new() -> Self {
        ...
        registry.register("net_udp_bind", 1, |vm, args| {
            ...
            let reference = vm.gc.allocate(GcObject::Sative(Box::new(socket)));
        ...
        registry.register("net_udp_send", 3, |vm, args| {
            ...
            let Some(GcObject::Sative(native)) = vm.gc.get_mut(socket_ref) else {
    ...
    {
        self.functions.push(SativeFunction {
            name,
