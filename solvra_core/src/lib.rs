...
//! SolvraCore v0.1 – a lightweight runtime and bytecode execution engine.
//!
...
//! * [`bytecode`] – definition of the SolvraBytecode format and an assembler capable of
//!   lowering a small high-level AST into a compact instruction stream.
//! * [`SolvraRuntime`] – an embeddable runtime with a cost-metered interpreter capable of
//!   executing the bytecode produced by the assembler.
//! * [`sys`] – cross platform utilities for IO and networking that SolvraRuntime and higher
//!   level applications can depend on.
//!
//! The implementation purposely focuses on being easy to audit and extend. The public API is
//! designed so other SolvraOS crates (CLI, IDE, or third party tools) can share the runtime
...
/// Result type used across SolvraCore.
pub type SolvraResult<T> = std::result::Result<T, SolvraError>;

/// Values manipulated by the SolvraCore runtime.
#[derive(Clone)]
pub enum Value {
    Null,
    Boolean(bool),
...
impl From<bytecode::spec::SolvraBytecodeError> for SolvraError {
    fn from(err: bytecode::spec::SolvraBytecodeError) -> Self {
        SolvraError::Bytecode(err.to_string())
    }
}
...
impl FailSafeState {
    ...
    fn authenticate(&self, passphrase: &str) -> Result<(), SolvraError> {
        let mut guard = self.inner.write();
        let Some(config) = guard.as_mut() else {
            return Err(SolvraError::FailSafeLocked);
        };
        let parsed = PasswordHash::new(&config.hash)
            ...
            .is_ok()
        {
            ...
            Err(SolvraError::FailSafeAuthFailed)
        }
    }

    fn ensure_unlocked(&self) -> Result<(), SolvraError> {
        let guard = self.inner.read();
        match guard.as_ref() {
            Some(cfg) if cfg.unlocked => Ok(()),
            Some(_) => Err(SolvraError::FailSafeLocked),
            None => Ok(()),
        }
    }
}
...
/// SolvraRuntime orchestrates backend selection, execution, and provides fail-safe gating.
#[derive(Debug, Clone)]
pub struct SolvraRuntime {
    ...
    vm_loader: vm::loader::ModuleLoaderVm,
}

impl Default for SolvraRuntime {
    fn default() -> Self {
        Self::new()
    }
}
...
impl SolvraRuntime {
    ...
    pub fn from_bootstrap(state: RuntimeBootstrapState) -> Self {
        ...
        let memory_for_vm = memory.clone();
        ...
        Self {
            ...
            memory,
            executor,
            vm_loader: vm::loader::ModuleLoaderVm::new(memory_for_vm),
        }
    }

    /// Overrides the cost limit used by the interpreter.
    pub fn with_cost_limit(mut self, cost_limit: u64) -> SolvraResult<Self> {
        if cost_limit == 0 {
            return Err(SolvraError::InvalidCostLimit(
    ...
    pub fn enable_failsafe(&self, passphrase: &str) -> SolvraResult<()> {
        self.failsafe
            .enable(passphrase)
            .map_err(|err| SolvraError::Internal(err.to_string()))
    }
    ...
    pub fn authenticate(&self, passphrase: &str) -> SolvraResult<()> {
        self.failsafe.authenticate(passphrase)
    }

    /// Executes the provided bytecode.
    #[instrument(skip_all)]
    pub fn execute(&self, bytes: &[u8]) -> SolvraResult<Value> {
    ...
    pub fn load_module_file(&self, path: impl AsRef<Path>) -> SolvraResult<Arc<Module>> {
        self.modules.write().load_file(path)
    }

    /// Loads a VM module from raw bytes.
    pub fn load_vm_module_from_memory(
        &self,
        bytes: &[u8],
    ) -> SolvraResult<vm::loader::LoadedModule> {
        self.vm_loader.load_from_bytes(bytes)
    }
    ...
    pub fn run_loop(&self) -> SolvraResult<()> {
        loop {
            match self.executor.poll_once() {
                LoopState::Idle => {
                    println!("[SolvraRuntime] loop idle - all tasks done");
                    break;
                }
                LoopState::Pending(remaining) => {
                    println!("[SolvraRuntime] active - {remaining} tasks remaining");
                }
            }
        }
        Ok(())
    ...
    pub fn execute_module(&self, module: Arc<Module>) -> SolvraResult<Value> {
        self.failsafe.ensure_unlocked()?;
        let backend = backend::active_backend();
        let artifact = backend.compile(module.bytecode())?;
        backend.execute(artifact, self.config.clone(), self.modules.clone())
    ...
    /// Loads a `.svc` file and schedules it for execution inside the runtime loop.
    pub fn load_vm_module(&self, path: impl AsRef<Path>) -> SolvraResult<vm::loader::LoadedModule> {
        self.vm_loader.load_from_path(path)
    }
}
...
mod tests {
    ...
    fn failsafe_blocks_execution() {
        ...
        let bytecode = assemble(&ast).expect("assemble");
        let runtime = SolvraRuntime::new();
        runtime.enable_failsafe("solvra").unwrap();
        let bytes = bytecode.into_bytes();
        let err = runtime.execute(&bytes).unwrap_err();
        assert_eq!(err, SolvraError::FailSafeLocked);
        runtime.authenticate("solvra").unwrap();
        runtime.execute(&bytes).unwrap();
    }

    #[test]
    fn cost_limit_validation() {
        assert!(SolvraRuntime::new().with_cost_limit(0).is_err());
    }
