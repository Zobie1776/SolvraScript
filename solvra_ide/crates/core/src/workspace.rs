...
impl WorkspaceLoader {
    ...
    pub fn load(&self) -> Result<WorkspaceConfig, SolvraIdeError> {
        if !self.path.exists() {
            return Ok(WorkspaceConfig::default());
        }
        let content = fs::read_to_string(&self.path)?;
        let mut config: WorkspaceConfig = toml::from_str(&content)
            .map_err(|err| SolvraIdeError::workspace(format!("invalid workspace config: {err}")))?;

        if let Some(project) = &mut config.project {
            if project.name.as_deref().unwrap_or_default().is_empty() {
                project.name = Some("SolvraIDE".into());
            }
        }
    ...
    pub fn save(&self, config: &WorkspaceConfig) -> Result<(), SolvraIdeError> {
        let content = toml::to_string_pretty(config).map_err(|err| {
            SolvraIdeError::workspace(format!("failed to serialize config: {err}"))
        })?;
    ...
    pub fn infer_from_path(&self, project_root: &Path) -> Result<WorkspaceConfig, SolvraIdeError> {
        if project_root.join("solvra.toml").exists() {
...
mod tests {
    ...
    fn saves_and_loads() {
        ...
        assert_eq!(roundtrip.task.default.build, "solvra build");
    }

    #[test]
    fn infer_uses_directory_name() {
        let tmp = tempdir().unwrap();
        std::fs::write(tmp.path().join("solvra.toml"), "[package]\nname='demo'\n").unwrap();
        let loader = WorkspaceLoader::new(tmp.path().join("workspace.toml"));
