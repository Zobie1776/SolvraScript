...
impl Default for ClientOptions {
    fn default() -> Self {
        Self {
            base_url: "https://api.solvraos.dev".into(),
            workspace: None,
...
impl SolvraAiClient {
    ...
    pub fn cycle_mode(&mut self) {
        ...
        info!("solvra_ai_mode {:?}", self.mode);
    }

    pub async fn request_completion(&mut self, prompt: &str) -> Result<String> {
        self.ensure_authenticated().await?;
        debug!("solvra_ai_prompt mode={:?} prompt={}", self.mode, prompt);
        let response = format!("[SolvraAI {:?}] {prompt}", self.mode);
    ...
    pub fn show_compact(&mut self, ui: &mut Ui) {
        ...
        for item in self.history.iter().rev().take(3) {
            ui.label(item.body.to_string());
        }
    }

    pub fn show_full(&mut self, ui: &mut Ui) {
        ui.heading("SolvraAI Workspace");
