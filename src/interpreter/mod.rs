...
use solvra_core::memory::{MemoryHandle, MemoryStats};
use solvra_core::sys::hal::{
    DeviceCapability, DeviceInfo, DeviceKind, HardwareAbstractionLayer, SensorKind, StorageBus,
};

//=============================================/*
//  Collects crate attributes and imports required by the SolvraScript interpreter runtime.
//============================================*/
//=============================================
...
/// SolvraScript runtime value types
#[derive(Debug, Clone)]
pub enum Value {
    Int(i64),
    Float(f64),
...
impl Value {
    /// Check if value is truthy (SolvraScript truthiness rules)
    //Function: is_truthy
    //Purpose: Evaluate SolvraScript truthiness semantics for conditional flow
    //Inputs: &self
    //Returns: bool
...
//=============================================/*
//  Encapsulates SolvraScript runtime values and helper methods for type coercion.
//============================================*/
//=============================================
...
//=============================================/*
//  Describes SolvraScript runtime errors and conversion helpers from system layers.
//============================================*/
//=============================================
...
pub struct Interpreter {
    ...
    hot_counts: HashMap<String, usize>,
    promoted_functions: HashSet<String>,
    next_handle_id: u64,
    http_agent: Agent,
...
impl Interpreter {
    ...
    pub fn with_loader(loader: SharedModuleLoader, core: Arc<CoreBridge>) -> Self {
        CoreBridge::install_global(core.clone());
        let hal = core.hal();
        let mut interpreter = Self {
            ...
            promoted_functions: HashSet::new(),
            next_handle_id: 1,
    ...
    pub fn with_std() -> Self {
        ...
        interpreter.initialize_standard_environment();
        interpreter
    }
    ...
    fn initialize_standard_environment(&mut self) {
        ...
        self.add_module_search_path(src_root.clone());
        self.add_module_search_path(src_root.join("stdx"));
    }

    pub fn script_search_paths(&self) -> Vec<PathBuf> {
        self.module_loader.borrow().script_search_paths()
    }
    ...
    pub fn reset_execution_timer(&mut self) {
        self.restart_timer(true);
    }

    fn restart_timer(&mut self, reset_instruction_counter: bool) {
        self.start_time = Instant::now();
        if reset_instruction_counter {
    ...
    fn builtin_endl(&mut self, _: &[Value]) -> Result<Value, RuntimeError> {
        // Provides a semantic newline emitter so SolvraScript code can mirror C++'s `std::endl`.
    ...
    //Purpose: Execute a SolvraScript program without origin tracking
    ...
    fn guard_execution(&mut self) -> Result<(), RuntimeError> {
        self.instruction_counter = self.instruction_counter.saturating_add(1);
        if self.instruction_counter > INSTRUCTION_LIMIT {
            return Err(RuntimeError::Custom(format!(
                "instruction limit of {} exceeded",
                INSTRUCTION_LIMIT
            )));
        }
        if self.start_time.elapsed() >= Duration::from_secs(TEST_TIMEOUT_SECS) {
            return Err(RuntimeError::Custom(format!(
                "test timeout of {} seconds exceeded",
                TEST_TIMEOUT_SECS
            )));
        }
        Ok(())
    ...
    fn eval_program_internal(&mut self, program: &Program) -> Result<Option<Value>, RuntimeError> {
        println!(
            "[debug] Interpreter eval_program_internal: {} statements, implicit_entry={}",
            program.statements.len(),
            program.implicit_entry
        );
        let mut last = None;
    ...
    fn eval_stmt(&mut self, stmt: &Stmt) -> Result<Option<Value>, RuntimeError> {
        ...
        match stmt {
            ...
            Stmt::While {
                ...
                println!("[debug] Interpreter entering while loop");
                let mut result = Ok(None);
                loop {
                    self.guard_execution()?;
                    let cond_val = self.eval_expr(condition)?;
        ...
        match &decl.item {
            ...
            ExportItem::Symbol { name, alias } => {
                ...
                if let Some(alias_name) = alias {
                    self.define_variable(alias_name.clone(), value, false);
                }
                Ok(None)
            }
    ...
    fn execute_import(&mut self, decl: &ImportDecl) -> Result<(), RuntimeError> {
        let base_dir = self.module_path_stack.last().map(|p| p.as_path());
        let key = self.module_loader.borrow_mut().get_key(&decl.source, base_dir)?;

        let exports = if let Some(exports) = self.module_loader.borrow().exports_cloned(&key) {
            exports
        } else {
            let descriptor = self.module_loader.borrow_mut().prepare_module(&decl.source, base_dir)?;
            
            if let Some(exports) = self.module_loader.borrow().exports_cloned(&key) {
                exports
            } else {
                let loaded_exports = self.evaluate_module_and_get_exports(&descriptor)?;
                self.module_loader.borrow_mut().store_exports(&descriptor.id, loaded_exports.clone());
                loaded_exports
            }
        };
    ...
    fn evaluate_module_and_get_exports(
        &mut self,
        descriptor: &ModuleDescriptor,
    ) -> Result<HashMap<String, Value>, RuntimeError> {
        match &descriptor.artifact {
            ModuleArtifact::Script { program, path, .. } => {
                let initial_globals: HashSet<String> = self.globals.keys().cloned().collect();

                let origin: PathBuf = path.clone();
                let _ = self.eval_program_with_origin(program, Some(origin.as_path()))?;
                
                let mut exports = HashMap::new();
                for (name, entry) in &self.globals {
                    if !initial_globals.contains(name) {
                ...
                if descriptor.declared_exports.is_empty() {
                    ...
                    let mut filtered_exports = HashMap::new();
                    for export_name in &descriptor.declared_exports {
                        if let Some(value) = exports.get(export_name) {
                            filtered_exports.insert(export_name.clone(), value.clone());
                        } else if let Some(value) = self.get_variable(export_name) {
                             filtered_exports.insert(export_name.clone(), value);
                        }
                    }
                    Ok(filtered_exports)
                }
    ...
    fn eval_expr(&mut self, expr: &Expr) -> Result<Value, RuntimeError> {
        ...
        match expr {
            ...
            Expr::Slice {
                ...
                self.eval_slice(target, start_val, end_val, step_val)
            }
            Expr::Member {
                object, property, ..
            } => {
            ...
            Expr::MethodCall {
                ...
                let mut with_receiver = Vec::with_capacity(arg_values.len() + 1);
                with_receiver.push(object_value);
                with_receiver.extend(arg_values.iter().cloned());

                match self.call_function(callable.clone(), with_receiver) {
                    Err(RuntimeError::ArgumentError(_)) => self.call_function(callable, arg_values),
                    result => result,
                }
            }
            Expr::Lambda {
    ...
    fn eval_index_assignment(
        ...
        match &mut array_value {
            Value::Array(items) => {
                let idx = self.expect_index_value(idx_value.clone())?;
                if idx >= items.len() {
                    return Err(RuntimeError::TypeError(format!(
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "Index assignment requires array or object target".to_string(),
                ));
            }
        }
    ...
    fn builtin_env_set(&mut self, args: &[Value]) -> Result<Value, RuntimeError> {
        ...
        // interacts with global process state. SolvraScript explicitly opts in
            ...
            if let Value::String(_) | Value::Null = entry {
                if !matches!(entry, Value::Null) {
                    return Err(RuntimeError::NotImplemented(
                        "Selecting custom SolvraCore entry points is not yet supported".into(),
                    ));
                }
            } else {
                return Err(RuntimeError::TypeError(
        ...
        match self.resources.remove(&id) {
            Some(Resource::CoreModule { memory, .. }) => {
                if !self.core.release(memory) {
                    return Err(RuntimeError::Custom(format!(
                        "Failed to release SolvraCore module handle {}",
                        id
                    )));
                }
                Ok(Value::Null)
            ...
            Some(_) => Err(RuntimeError::TypeError(format!(
                "Handle {} does not reference a SolvraCore module",
                id
            ))),
    ...
    fn builtin_core_value_release(&mut self, args: &[Value]) -> Result<Value, RuntimeError> {
        ...
        match self.resources.remove(&id) {
            Some(Resource::CoreValue { memory, .. }) => {
                if !self.core.release(memory) {
                    return Err(RuntimeError::Custom(format!(
                        "Failed to release SolvraCore allocation handle {}",
                        id
                    )));
                }
                Ok(Value::Null)
            ...
            Some(_) => Err(RuntimeError::TypeError(format!(
                "Handle {} is not a SolvraCore allocation",
                id
            ))),
            None => Err(RuntimeError::ArgumentError(format!(
                "Unknown handle {}",
    ...
    fn core_module_handle(&self, id: u64) -> Result<MemoryHandle, RuntimeError> {
        match self.resources.get(&id) {
            ...
            Some(_) => Err(RuntimeError::ArgumentError(format!(
                "Handle {} is not a SolvraCore module",
                id
            ))),
            None => Err(RuntimeError::ArgumentError(format!(
                "Unknown module handle {}",
    ...
    fn wrap_core_allocation(
        ...
        let handle = self.core.allocate_value(value).map_err(|err| {
            RuntimeError::Custom(format!("SolvraCore allocation failed: {}", err))
        })?;
    ...
    fn eval_slice(
        ...
        let step_value = match parse_slice_step(&step) {
            Ok(v) => v,
            Err(err) => return Err(err),
        };
        let start_idx = self.parse_slice_index(&start)?;
        let end_idx = self.parse_slice_index(&end)?;

        match target {
            Value::Array(items) => {
                let indices = compute_slice_indices(items.len(), start_idx, end_idx, step_value)?;
            ...
            Value::String(text) => {
                ...
                let indices = compute_slice_indices(chars.len(), start_idx, end_idx, step_value)?;
    ...
    fn parse_slice_index(&self, value: &Value) -> Result<Option<i64>, RuntimeError> {
        match value {
            Value::Null => Ok(None),
    ...
    fn call_function(&mut self, func: Value, args: Vec<Value>) -> Result<Value, RuntimeError> {
        ...
        match func {
            ...
            Value::Function {
                ...
                if !name.is_empty() && !self.promoted_functions.contains(&name) {
                    let entry = self.hot_counts.entry(name.clone()).or_insert(0);
                    *entry += 1;
                    if *entry >= HOT_CALL_THRESHOLD && params.is_empty() {
                        // VM entry routines currently expect zero arguments, so only promote
                        // functions without parameters to avoid arity mismatches.
                        match vm_compiler::compile_function_from_parts(&name, &params, &body) {
                            Ok(bytes) => {
                                if let Err(err) =
                                    CoreBridge::global().load_vm_module_from_memory(bytes)
                                {
                                    eprintln!("failed to promote function '{name}' to VM: {err}");
                                } else {
                                    self.promoted_functions.insert(name.clone());
                                }
                            }
                            Err(err) => eprintln!("VM compilation for '{name}' failed: {err}"),
                        }
                    }
                }
    ...
    //=============================================/*
    //  Executes SolvraScript AST nodes, handling statements, expressions, and control flow.
    //============================================*/
    //=============================================
...
/// Write SolvraScript values to stdout while preserving their exact textual representation.
///
...
fn parse_slice_step(value: &Value) -> Result<i64, RuntimeError> {
    match value {
        Value::Null => Ok(1),
        Value::Int(0) => Err(RuntimeError::TypeError(
            "Slice step cannot be zero".to_string(),
        )),
        Value::Int(v) => Ok(*v),
        other => Err(RuntimeError::TypeError(format!(
            "Slice step must be integer, got {}",
            other.type_name()
        ))),
    }
}
...
//=============================================/*
//  Provides IO helpers, numeric coercions, and conversions between SolvraScript values and JSON.
//============================================*/
//=============================================
//            Section 11: Tests
//=============================================
...
mod tests {
    ...
    fn test_hal_read_write_cycle() {
        ...
        let mut writable_handle: Option<String> = None;
        for entry in &list {
            ...
            if result.is_ok() {
                writable_handle = Some(handle.clone());
                break;
            }
        }

        let handle = writable_handle.expect("expected at least one writable HAL device");
    ...
    fn test_file_roundtrip() {
        ...
        path.push("solvrascript_test.txt");
...
//============================================*/
//=============================================
// End Of solvra_script/interpreter.rs
//=============================================
// Notes:
// -[@ZNOTE] Keep interpreter builtins synchronized with SolvraCore capability surface.
//=============================================
