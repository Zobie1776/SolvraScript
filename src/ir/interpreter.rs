...
fn slice_runtime_value(
    object: &RuntimeValue,
    start: &RuntimeValue,
    end: &RuntimeValue,
    step: &RuntimeValue,
) -> Result<RuntimeValue, IrInterpreterError> {
    let step_value = match step.as_int() {
        Some(0) => {
    ...
    let slice_indices = |len: usize| -> Result<Vec<usize>, IrInterpreterError> {
        let len_i = len as i64;
        let normalize = |idx: i64| if idx < 0 { idx + len_i } else { idx };
        let default_start = if step_value > 0 { 0 } else { len_i - 1 };
        let default_end = if step_value > 0 { len_i } else { -1 - len_i };

        let start_idx = normalize(start.as_int().unwrap_or(default_start));
        let end_idx = normalize(end.as_int().unwrap_or(default_end));

        let start_idx = if step_value > 0 {
            start_idx.clamp(0, len_i)
        } else {
            start_idx.clamp(-1, len_i - 1)
        };
        let end_idx = if step_value > 0 {
            end_idx.clamp(0, len_i)
        } else {
            end_idx.clamp(-1, len_i)
        };
    ...
    match object {
        RuntimeValue::Array(items) => {
            let data = items.borrow();
            let indices = slice_indices(data.len())?;
        ...
        RuntimeValue::String(text) => {
            ...
            let indices = slice_indices(chars.len())?;
        ...
        _ => Err(IrInterpreterError::Runtime(
        ...
        loop {
            ...
            for instruction in &block.instructions {
                let result = match &instruction.opcode {
                    ...
                    IrOpcode::Slice => {
                        let array = self.load(&values, instruction.operands[0])?;
                        let start = self.load(&values, instruction.operands[1])?;
                        let end = self.load(&values, instruction.operands[2])?;
                        let step = self.load(&values, instruction.operands[3])?;
                        slice_runtime_value(&array, &start, &end, &step)?
                    }
                    IrOpcode::ObjectNew => {
