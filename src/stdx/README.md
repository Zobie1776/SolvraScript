...
```bash
# Run stdx .svs integration suites
cargo test -p solvrascript --test math -- --test-threads=1 --nocapture
cargo test -p solvrascript --test string
cargo test -p solvrascript --test path
cargo test -p solvrascript --test datetime
cargo test -p solvrascript --test fs
cargo test -p solvrascript --test stdlib
```
...
cargo run -p solvrascript -- src/stdx/examples/web_min_server.svs

# Run ECS Pong demo
cargo run -p solvrascript -- src/stdx/examples/game_ecs_pong.svs

# Run JWT demo
cargo run -p solvrascript -- src/stdx/examples/sec_jwt_roundtrip.svs

# Run CSV pipeline
cargo run -p solvrascript -- src/stdx/examples/data_csv_pipeline.svs
```
