...
const MAX_TOKENIZER_STEPS: usize = 1_000_000;
...
/// All possible token types in SolvraScript
#[derive(Debug, Clone, PartialEq)]
pub enum TokenKind {
    // Literals
    Integer(i64),
...
impl TokenKind {
    ...
    /// string payloads.
    #[allow(dead_code)] // Primarily used in tests and tooling queries.
...
impl Tokenizer {
    pub fn new(input: &str) -> Self {
        ...
        keywords.insert("function".to_string(), TokenKind::Fn);
    ...
    pub fn tokenize(&mut self) -> Result<Vec<Token>, String> {
        ...
        while !self.is_at_end() {
            steps += 1;
            if steps > MAX_TOKENIZER_STEPS {
                return Err(
                    "Tokenizer exceeded maximum step limit; possible infinite loop".to_string(),
                );
            }
...
// @ZNOTE[SolvraCore Integration]: This tokenizer will eventually feed into the SolvraCore compiler
// pipeline. The Token structure is designed to be serializable for cross-module communication.

// @ZNOTE[SolvraStdLib Hook]: String interpolation tokens will need to interface with SolvraStdLib's
...
print("Result: ${x}");"#;
    ...
    fn test_compound_assignment_tokens() {
        ...
        let expected_kinds = vec![
            ...
            TokenKind::PlusEqual,
            TokenKind::Integer(1),
            TokenKind::Identifier(intern_symbol("y")),
            TokenKind::MinusEqual,
            TokenKind::Integer(2),
            TokenKind::Identifier(intern_symbol("z")),
            TokenKind::StarEqual,
            TokenKind::Integer(3),
            TokenKind::Identifier(intern_symbol("w")),
            TokenKind::SlashEqual,
            TokenKind::Integer(4),
            TokenKind::Eof,
        ];
    ...
    #[test]
    fn test_keyword_boolean_operators() {
        let mut tokenizer = Tokenizer::new("and or not is elif");
        let tokens = tokenizer.tokenize().unwrap();
    ...
    fn test_position_tracking() {
        ...
        let x_token = tokens
            .iter()
            .find(|t| matches!(t.kind, TokenKind::Identifier(ref name) if name.as_str() == "x"))
            .unwrap();
