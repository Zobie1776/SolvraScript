...
impl<'a> FunctionCompiler<'a> {
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Result<()> {
        match expr {
            ...
            Expr::Slice {
                ...
                self.compile_optional_index(start.as_deref())?;
                self.compile_optional_index(end.as_deref())?;
                self.compile_optional_index(step.as_deref())?;
                let name_index = self
                    .program
                    .constant_index(Constant::String("__slice".to_string()));
                self.emit_instruction(Opcode::CallBuiltin, &[name_index, 4]);
                Ok(())
            }
            Expr::Binary {
                left,
    ...
    fn compile_optional_index(&mut self, expr: Option<&Expr>) -> Result<()> {
